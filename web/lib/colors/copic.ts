...
// Variant maps per Copic line from BSale (generated by /api/bsale/build-all-maps).
// Each map: { [colorCode]: bsaleVariantId }
const VARIANT_MAPS = (bsaleVariants as {
  by_brand: { 'copic-sketch': Record<string, number>; 'copic-ink': Record<string, number>; 'copic-ciao': Record<string, number> };
}).by_brand;
const SKETCH_VARIANT_MAP = VARIANT_MAPS['copic-sketch'];
const INK_VARIANT_MAP = VARIANT_MAPS['copic-ink'];
const CIAO_VARIANT_MAP = VARIANT_MAPS['copic-ciao'];
...
function withImages(
  ...
  return Object.keys(images)
    .map(code => ({
      ...
      variantId: variantMap?.[code],
    }))
    .sort(sort);
}

// Each Copic line has its OWN bsaleProductId and OWN set of variantIds.
// Sketch and Ink were aliased before but their BSale variantIds differ.
export const COPIC_SKETCH: ColorSwatch[] = withImages(sketchImages as Record<string, string>, SKETCH_VARIANT_MAP);
export const COPIC_INK: ColorSwatch[] = withImages(sketchImages as Record<string, string>, INK_VARIANT_MAP);
export const ALL_COPIC = COPIC_SKETCH; // legacy alias for code that doesn't care
...
// AND the Ciao-specific BSale variantId map (different from Sketch's).
const ciaoMap = ciaoImages as Record<string, string>;
export const COPIC_CIAO: ColorSwatch[] = withImages(ciaoMap, CIAO_VARIANT_MAP)
  .filter(c => CIAO_SUBSET.has(c.code) || ciaoMap[c.code]);
