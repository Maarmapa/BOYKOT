...
interface MegaCategory {
  label: string;
  href: string;
  // Optional small intro shown on the left panel
  blurb?: string;
  // Featured items (with image thumbnails) rendered as a grid on the right
  featured?: MegaItem[];
  // Additional plain links shown below the featured grid
  links?: Array<{ label: string; href: string }>;
}

// Single-link nav entries (no dropdown)
interface NavLink {
  label: string;
  href: string;
}
...
const NAV: NavEntry[] = [
  {
    ...
    featured: [
      ...
      {
        label: 'Molotow',
        href: '/categoria/molotow-markers',
        hint: 'One4All',
  ...
  },
  {
    ...
    featured: [
      ...
      {
        label: 'Holbein Acuarela',
        href: '/colores/holbein-acuarela-15ml',
        hint: '120 colores',
      ...
      {
        label: 'Molotow Premium',
        href: '/colores/molotow-premium',
        hint: 'Aerosol 400ml',
        image: 'https://www.boykot.cl/wp-content/uploads/2024/10/p00327000-ef9dadfd-968f-4c16-bd8d-ca991af72c30.jpg',
      },
      {
        label: 'Createx Airbrush',
      ...
      {
        label: 'Gouache Holbein',
        href: '/colores/holbein-gouache-15ml',
        hint: '105 colores',
        image: 'https://www.boykot.cl/wp-content/uploads/2025/07/170908red_tex_new_fx-ec0ab6c9-e1dc-4f63-af58-b2e177b4ece3-4.jpg',
      },
    ],
        ...
        {NAV.map((item, i) => {
          ...
          return (
            ...
            >
              <Link
                href={item.href}
                className={`font-medium ${openIdx === i ? 'text-gray-900' : 'text-gray-700 hover:text-gray-900'}`}
                onFocus={() => hasMega && open(i)}
              >
                {item.label}
                {hasMega && <span className="ml-1 text-gray-400">▾</span>}
              </Link>
            </li>
          );
        })}
